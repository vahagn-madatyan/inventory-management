<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-2xl font-semibold tracking-tight">{{ t("finance.title") }}</h1>
      <p class="text-sm text-muted-foreground">{{ t("finance.description") }}</p>
    </div>
    ...
    <div v-else class="space-y-6">
      ...
      <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card class="border-l-4 border-l-slate-900">
          <CardHeader class="pb-2">
            ...
            <CardTitle class="text-2xl">{{ formatCurrency(revenueMetrics.totalRevenue) }}</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card class="border-l-4 border-l-red-500">
          <CardHeader class="pb-2">
            ...
            <CardTitle class="text-2xl">{{ formatCurrency(totalCosts) }}</CardTitle>
          </CardHeader>
          <CardContent>
            <p class="mt-1 text-xs text-muted-foreground">{{ t("finance.costBreakdown") }}</p>
          </CardContent>
        </Card>
        ...
        <Card class="border-l-4 border-l-blue-500">
          <CardHeader class="pb-2">
            ...
            <CardTitle class="text-2xl">{{ formatCurrency(netProfit) }}</CardTitle>
          </CardHeader>
          <CardContent>
            <p class="mt-1 text-xs text-muted-foreground">{{ profitMargin }}% {{ t("finance.margin") }}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader class="pb-2">
            ...
            <CardTitle class="text-2xl">{{ formatCurrency(revenueMetrics.avgOrderValue) }}</CardTitle>
          </CardHeader>
          <CardContent>
            <p class="mt-1 text-xs text-muted-foreground">{{ t("finance.perOrderRevenue") }}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle class="text-base font-semibold">{{ t("finance.revenueVsCosts.title") }}</CardTitle>
          ...
          <div class="py-4">
            <div class="flex gap-6 h-[350px]">
              ...
              <div class="flex flex-1 items-end justify-around gap-2">
                ...
                >
                  <div class="flex w-full max-w-[80px] gap-1.5 justify-center items-end h-full pb-8">
                    <div
                  ...
                  <span class="mt-2 text-xs font-semibold text-muted-foreground">{{ translateMonth(month.month) }}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <Card>
        <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle class="text-base font-semibold">{{ t("finance.monthlyCostFlow.title") }}</CardTitle>
          <div class="flex gap-4 text-sm">
            <span class="flex items-center gap-2 text-muted-foreground">
            ...
            <span class="flex items-center gap-2 text-muted-foreground">
              <span class="inline-block h-3 w-3 rounded-sm bg-violet-500"></span>
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle class="text-base font-semibold">{{ t("finance.categorySpending.title") }}</CardTitle>
          </CardHeader>
          <CardContent class="space-y-5">
            ...
            >
              <div class="flex items-center justify-between">
                <span class="font-semibold text-slate-900">{{ translateCategory(category.category) }}</span>
                <span class="text-lg font-bold text-blue-600">{{ currencySymbol }}{{ category.amount.toLocaleString() }}</span>
              </div>
              ...
              <div class="flex justify-between text-xs">
                <span class="text-muted-foreground">{{ category.percentage }}% {{ t("finance.categorySpending.ofTotal") }}</span>
                <span
                  class="font-semibold"
                  :class="{
        ...
        <Card class="flex flex-col">
          <CardHeader>
            <CardTitle class="text-base font-semibold">{{ t("finance.transactions.title") }}</CardTitle>
          </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{{ t("finance.transactions.vendor") }}</TableHead>
                    <TableHead>{{ t("finance.transactions.date") }}</TableHead>
                    <TableHead class="text-right">{{ t("finance.transactions.amount") }}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  <TableRow
                  ...
                  >
                    ...
                    <TableCell class="text-muted-foreground">{{ transaction.vendor }}</TableCell>
