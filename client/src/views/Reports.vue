<template>
  <div class="space-y-6">
    <div>
      ...
      <p class="text-sm text-muted-foreground">{{ t('reports.description') }}</p>
    </div>
    ...
    <template v-else>
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-base font-semibold">{{ t('reports.quarterly.title') }}</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{{ t('reports.quarterly.totalOrders') }}</TableHead>
                  <TableHead>{{ t('reports.quarterly.totalRevenue') }}</TableHead>
                  <TableHead>{{ t('reports.quarterly.avgOrderValue') }}</TableHead>
                  <TableHead>{{ t('reports.quarterly.fulfillmentRate') }}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                <TableRow v-for="q in quarterlyData" :key="q.quarter">
                  ...
                  <TableCell>{{ formatCurrencyWithDecimals(q.total_revenue, currentCurrency) }}</TableCell>
                  <TableCell>{{ formatCurrencyWithDecimals(q.avg_order_value, currentCurrency) }}</TableCell>
                  <TableCell>
                    <Badge
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-base font-semibold">{{ t('reports.monthlyTrend.title') }}</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              >
                <div class="h-[200px] flex items-end w-full">
                  <div
                    ...
                    :title="formatCurrencyWithDecimals(month.revenue, currentCurrency)"
                  ></div>
                </div>
                <div class="text-xs text-muted-foreground text-center -rotate-45 whitespace-nowrap mt-6">{{ formatMonth(month.month) }}</div>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-base font-semibold">{{ t('reports.monthOverMonth.title') }}</CardTitle>
        </CardHeader>
        <CardContent>
          <div class="rounded-md border">
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{{ t('reports.monthOverMonth.orders') }}</TableHead>
                  <TableHead>{{ t('reports.monthOverMonth.revenue') }}</TableHead>
                  <TableHead>{{ t('reports.monthOverMonth.change') }}</TableHead>
                  <TableHead>{{ t('reports.monthOverMonth.growthRate') }}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                <TableRow v-for="(month, index) in monthlyData" :key="month.month">
                  <TableCell class="font-semibold">{{ formatMonth(month.month) }}</TableCell>
                  <TableCell>{{ month.order_count }}</TableCell>
                  <TableCell>{{ formatCurrencyWithDecimals(month.revenue, currentCurrency) }}</TableCell>
                  <TableCell>
                    <span
                      v-if="index > 0"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent class="pt-6">
            <p class="text-sm text-muted-foreground">{{ t('reports.summary.totalRevenueYTD') }}</p>
            <p class="text-2xl font-bold tracking-tight mt-1">{{ formatCurrencyWithDecimals(totalRevenue, currentCurrency) }}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent class="pt-6">
            <p class="text-sm text-muted-foreground">{{ t('reports.summary.avgMonthlyRevenue') }}</p>
            <p class="text-2xl font-bold tracking-tight mt-1">{{ formatCurrencyWithDecimals(avgMonthlyRevenue, currentCurrency) }}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent class="pt-6">
            <p class="text-sm text-muted-foreground">{{ t('reports.summary.totalOrdersYTD') }}</p>
            <p class="text-2xl font-bold tracking-tight mt-1">{{ totalOrders }}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent class="pt-6">
            <p class="text-sm text-muted-foreground">{{ t('reports.summary.bestQuarter') }}</p>
            <p class="text-2xl font-bold tracking-tight mt-1">{{ bestQuarter }}</p>
          </CardContent>
        </Card>
      </div>
    </template>
