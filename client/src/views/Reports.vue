    ...
    <div v-else>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="q in quarterlyData" :key="q.quarter">
                ...
                <td>{{ formatCurrencyWithDecimals(q.total_revenue, currentCurrency) }}</td>
                <td>{{ formatCurrencyWithDecimals(q.avg_order_value, currentCurrency) }}</td>
                <td>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="(month, index) in monthlyData" :key="month.month">
                ...
                <td>{{ formatCurrencyWithDecimals(month.revenue, currentCurrency) }}</td>
                <td>
                  <span
                    v-if="index > 0"
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-value">{{ formatCurrencyWithDecimals(totalRevenue, currentCurrency) }}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">{{ t('reports.summary.avgMonthlyRevenue') }}</div>
          <div class="stat-value">{{ formatCurrencyWithDecimals(avgMonthlyRevenue, currentCurrency) }}</div>
        </div>
        <div class="stat-card">
...
export default {
  ...
  setup() {
    ...
    // Computed: max revenue for bar chart scaling — cached until monthlyData changes
    const maxRevenue = computed(() => {
      return monthlyData.value.reduce((max, m) => Math.max(max, m.revenue), 0);
    });

    // Computed summary stats — derived from reactive data refs
    ...
    // Bar height uses the computed maxRevenue (not recalculated on every call)
